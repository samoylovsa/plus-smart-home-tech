...
@Slf4j
@Service
public class CollectorKafkaProducerService {
    ...
    public void init() {
        ...
        log.info("Collector Kafka Producer initialized");
    }
    ...
    public void cleanup() {
        if (producer != null) {
            try {
                producer.flush();
                producer.close();
                log.info("Collector Kafka Producer closed");
            } catch (Exception e) {
                log.warn("Error closing Collector Kafka Producer", e);
            }
        }
    }
